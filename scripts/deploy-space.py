...
def run(nre: NileRuntimeEnvironment):
    ...
    nre.compile(
        ["contracts/core/random/random.cairo", "contracts/core/space/space.cairo"]
    )
    ...
    print(
        f"Deploying Space contract with params: space_size={space_size} turn_count={turn_count} max_dust={max_dust}"
    )
    spaceAddress, abi = nre.deploy("space")
    nre.invoke(
        spaceAddress,
        "initialize",
        [f"{randAddress}", f"{space_size}", f"{turn_count}", f"{max_dust}"],
    )
